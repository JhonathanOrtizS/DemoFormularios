...
{   
	...
	public function index(){
		...
		$blog = Blog::all();
        //llamamos a todos los usuarios registrados o administradores
    ...
    public function update($id, Request $request){
        ...
        //Fin recolectando datos


    	// Validar los datos
    	if(!empty($datos)){

            //En esta variable se va a validar todo lo que se esta
            //recolectando en $datos
             ...
             * Validar Imagenes
             */
            //Validar imágen de recoleccion del logo
            ...
            //Validar imágen recolectada de portada
    		...
    		if($validar->fails()){
                ...
                if($logo["logo_temporal"] != ""){
                    ...
                    //para eliminar la ruta del logo
                    unlink($datos["logo_actual"]);

                    $aleatorio = mt_rand(100,999);

                    //se asigna la ruta del archivo
                    //se asigna un valor aleatorio como nombre de archivo
                    //se concatena el valor que trae logo temporal y se obtiene la extensión
                    $rutaLogo = "img/blog/".$aleatorio.".".$logo["logo_temporal"]->guessExtension();

                    // esto se usa para archivos que no se necesitan redimencionar
                ...
                //recorra todos los archivo que encuentre en origen
                //img/blog
                foreach($origen as $fichero){

                    //va a copiar cada uno de los ficheros, para pasarlos a la 
                    //carpeta img/blog
                    //fichero trae toda la ruta desde img/temp/blog/
                    //por lo que se aplica substraer para usar solo el nombre del archivo
                    copy($fichero, "img/blog/".substr($fichero, 14));

                    //se borraran cada uno de los ficheros que traiga el forech
                    //de los archivos temp al destino final
                    unlink($fichero);
    			...
    			$actualizar = array("dominio"=> $datos["dominio"],
                                    ...
                                    "logo"=>$rutaLogo,
                                    "icono"=>$rutaIcono,
                                    //se usa str_replace() para reemplazar la direccion completa de la carpeta temporal
                                    //por la ruta sin la carpeta temporal
                                    //que es lo que trae $datos["sobre_mi"] de la base de datos
